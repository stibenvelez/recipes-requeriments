import { useRouter } from "next/router";
import React, { useEffect, useState } from "react";

const Likes = () => {
    const [likes, setLikes] = useState([]);
    const [like, setLike]= useState("")
    const { query } = useRouter();

    const handleLike = (e) => {
        setLikes([...likes, { id: query.id, like: e.target.name, date: Date()}]);
        localStorage.setItem(
            "likes",
            JSON.stringify([
                ...likes,
                { id: query.id, like: e.target.name, date: Date() },
            ])
        );

    };

    useEffect(() => {
        if (typeof window !== "undefined") {
            const commnetsLS = JSON.parse(localStorage.getItem("likes")) ?? [];
            setLikes(commnetsLS);
            return;
        }
        setLikes([]);
    }, []);

    useEffect(() => {
        const likeCurrent = likes.filter((item) => item.id === query.id);        
        setLike(likeCurrent.at(-1)?.like)
    }, [likes]);
    
    return (
        <section>
            <form>
                <div className="flex gap-4  items-center py-4">
                    <button
                        className={`bg-gray-200  hover:bg-green-100 p-2 rounded-full flex items-center justify-center ${
                            like === "like" ? "bg-green-500" : "bg-gray-200"
                        }`}
                        type="button"
                        name="like"
                        onClick={handleLike}
                    >
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                            fill="none"
                            viewBox="0 0 24 24"
                            stroke-width="1.5"
                            stroke="currentColor"
                            class="w-6 h-6"
                        >
                            <path
                                stroke-linecap="round"
                                stroke-linejoin="round"
                                d="M5.383 12.25c.806 0 1.533-.446 2.031-1.08a9.04 9.04 0 012.861-2.4c.723-.384 1.35-.956 1.653-1.715a4.498 4.498 0 00.322-1.672V4.75A.75.75 0 0113 4a2.25 2.25 0 012.25 2.25c0 1.152-.26 2.243-.723 3.218-.266.558.107 1.282.725 1.282m0 0h3.126c1.026 0 1.945.694 2.054 1.715.045.422.068.85.068 1.285a11.95 11.95 0 01-2.649 7.521c-.388.482-.987.729-1.605.729H12.23c-.483 0-.964-.078-1.423-.23l-3.114-1.04a4.501 4.501 0 00-1.423-.23H4.654m10.598-9.75H13M4.654 20.5c.083.205.173.405.27.602.197.4-.078.898-.523.898h-.908c-.889 0-1.713-.518-1.972-1.368A12 12 0 011 17.125c0-1.553.295-3.036.831-4.398.306-.774 1.086-1.227 1.918-1.227h1.053c.472 0 .745.556.5.96A8.958 8.958 0 004 17.124c0 1.194.232 2.333.654 3.375z"
                            />
                        </svg>
                    </button>
                    -
                    <button
                        className={` hover:bg-red-100 p-2 rounded-full flex items-center justify-center ${
                            like === "dislike" ? "bg-red-400" : "bg-gray-200"
                        }`}
                        type="button"
                        name="dislike"
                        onClick={handleLike}
                    >
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                            fill="none"
                            viewBox="0 0 24 24"
                            stroke-width="1.5"
                            stroke="currentColor"
                            class="w-6 h-6"
                        >
                            <path
                                stroke-linecap="round"
                                stroke-linejoin="round"
                                d="M7.498 14.25H4.372c-1.026 0-1.945-.694-2.054-1.715a12.137 12.137 0 01-.068-1.285c0-2.848.992-5.464 2.649-7.521C5.287 3.247 5.886 3 6.504 3h4.016a4.5 4.5 0 011.423.23l3.114 1.04a4.5 4.5 0 001.423.23h1.294M7.498 14.25c.618 0 .991.724.725 1.282A7.471 7.471 0 007.5 18.75 2.25 2.25 0 009.75 21a.75.75 0 00.75-.75v-.633c0-.573.11-1.14.322-1.672.304-.76.93-1.33 1.653-1.715a9.04 9.04 0 002.86-2.4c.498-.634 1.226-1.08 2.032-1.08h.384m-10.253 1.5H9.7m8.075-9.75c.01.05.027.1.05.148.593 1.2.925 2.55.925 3.977 0 1.487-.36 2.89-.999 4.125m.023-8.25c-.076-.365.183-.75.575-.75h.908c.889 0 1.713.518 1.972 1.368.339 1.11.521 2.287.521 3.507 0 1.553-.295 3.036-.831 4.398-.306.774-1.086 1.227-1.918 1.227h-1.053c-.472 0-.745-.556-.5-.96a8.95 8.95 0 00.303-.54"
                            />
                        </svg>
                    </button>
                </div>
            </form>
        </section>
    );
};

export default Likes;
